#!/usr/bin/env python3
"""Enrich the 530 new active reports with metrics, tables, and filter definitions.
Uses /model/reports/{id} endpoint - fast, no SQL execution needed.
"""
...
from mstr_report_inventory import MstrClient, extract_filter, extract_units, parse_table_names_from_sql

DATA_DIR = Path("Global Operational")
INPUT_FILE = DATA_DIR / "inventory" / "reports_new_in_telemetry.json"
OUTPUT_FILE = DATA_DIR / "inventory" / "reports_new_enriched.json"
...
def main():
    ...
    print(f"Enriching {len(reports)} new active reports...\n", flush=True)
    ...
    for i, rec in enumerate(reports, 1):
        ...
        try:
            ...
            if not resp.ok:
                ...
                success += 1
                status = f"OK (metrics:{len(metrics)}, filter:{'Y' if rec.get('filter') else 'N'})"
